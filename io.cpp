   ...
   do {
      ...
      switch (key = getch()) {
         case 'w': //Wear Item
            clear();
            list_inventory(d->PC);
            mvprintw(0,0, "Please select an inventory slot (0-9) that contains the item you wish to equip.");

            sub_key = getch() - 48;
            if(sub_key >= 0 && sub_key <= 9)
            {
               equip_item(d->PC, sub_key);
            }
            else
            {
               io_queue_message("That was out of the range of slots.");
            }

            io_queue_message("Press any key to continue...");
         ...
         case 't': // Take off an item
            clear();
            list_equipment(d->PC);
            mvprintw(0, 0, "Pick an inventory slot to unequip!");

            // Covert the character so that 'a' is 0, 'b' is 1, 5etc.
            sub_key = getch() - 97;

            if(sub_key >= 0 && sub_key <= 11)
            {
               unequip_item(d->PC, sub_key);
               io_queue_message("Press any key to continue...");
            }
            else
            {
               io_queue_message("That was out of the range of slots. Please try again. Press any key to continue...");
            }
         ...
         case 'd': // Drop an item
            clear();
            list_inventory(d->PC);
            mvprintw(0,0, "Pick an inventory slot to drop!");
            sub_key = getch() - 48;
            if(sub_key >= 0 && sub_key <= 9)
            {
               drop_item(d, sub_key);
            ...
            break;
         case 'x': // Expunge an item
            clear();
            list_inventory(d->PC);
            mvprintw(0,0, "Select an inventory slot (1-9) that you would like to expunge.");
            sub_key = getch() - 48;
            if(sub_key >= 0 && sub_key <= 9)
            {
               if(d->PC->inventory[sub_key])
               {
                  d->PC->inventory[sub_key] = NULL; // Setting this too null removes the last reference to it
                  io_queue_message("The item in slot %d was expunged! Press any key to continue...", sub_key);
               }
               else
               {
                  io_queue_message("There was no item to expunge in slot %d. Press any key to continue...", sub_key);
         ...
         case 'i': // List PC inventory
            clear();
            list_inventory(d->PC);
            mvprintw(0,0, "Current Inventory: ");
            getch();
            break;
         case 'e': // List PC equipment
            clear();
            list_equipment(d->PC);
            mvprintw(0,0, "Current Equipment Loadout: ");
            getch();
            break;
         case 'I':
             // Inspect an item
            ...
            if(sub_key >= 0 && sub_key <= 9)
            {
               if(d->PC->inventory[sub_key])
               {
                  io_queue_message("%s", d->PC->inventory[sub_key]->get_description().c_str());
               }
               else
               {
                  io_queue_message("There is nothing in that slot!");
               }
