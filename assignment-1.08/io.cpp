...
void io_display(dungeon_t *d)
{
  ...
  for (y = 0; y < 21; y++) {
    ...
    attroff(COLOR_PAIR(d->character_map[d->io_offset[dim_y] + y]
                              ...
                              [d->io_offset[dim_x] + x]) {
    attron(COLOR_PAIR(d->object_map[d->io_offset[dim_y] + y]
                              [d->io_offset[dim_x] + x]->color));
        mvaddch(y + 1, x, (chtype)(d->object_map[d->io_offset[dim_y] + y]
                        [d->io_offset[dim_x] + x])->symbol);
    attroff(COLOR_PAIR(d->object_map[d->io_offset[dim_y] + y]
                              [d->io_offset[dim_x] + x]->color));
      } else {
        switch (d->map[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x]) {
        case ter_wall:
