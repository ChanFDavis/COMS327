...
void gen_objects(dungeon_t *d)
{
  ...
  for(i = 0; i < NUM_OBJECTS; i++)
  {
    ...
    desc = d->object_descriptions[rand_range(0, d->object_descriptions.size())];

    o->color=desc.get_color();
    o->hit=desc.get_hit().roll();
    o->dodge=desc.get_dodge().roll();
    o->defence=desc.get_defence().roll();
    o->weight=desc.get_weight().roll();
    o->speed=desc.get_speed().roll();
    o->attribute=desc.get_attribute().roll();
    o->value=desc.get_value().roll();
    o->damage=desc.get_damage();
    ...
    for (j = 0; colors_lookup[j].name; j++) {
      if (desc.get_type() == colors_lookup[j].value) {
        o->symbol = object_symbol[j];
        break;
      }
    }
