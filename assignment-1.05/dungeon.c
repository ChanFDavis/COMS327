...
void render_dungeon(dungeon_t *d)
{
	...
	for (int y = 1 ; y < 21; y++)
	{
		for (int x = 0; x < 80; x++)
		{
			...
			if(d->pc.position[dim_x] + (x - 40) < 0 ||
			   d->pc.position[dim_x] + (x - 40) >= 160 ||
			   d->pc.position[dim_y] + (y - 11) < 0 ||
			   d->pc.position[dim_y] + (y - 11) >= 105)
			{
				addch(' ');
			} 
			else if(charxy(d->pc.position[dim_x] + (x - 40), d->pc.position[dim_y] + (y - 11)) )
			{	
				addch(charxy(d->pc.position[dim_x] + (x - 40), d->pc.position[dim_y] + (y - 11))->symbol );
			}
			else
			{
				switch(mapxy(d->pc.position[dim_x] + (x - 40),d->pc.position[dim_y] + (y - 11))) 
				{
					case ter_wall:
...
void look_render(dungeon_t *d)
{
	for (int y = 1 ; y < 21; y++)
	{
		for (int x = 0; x < 80; x++)
		{
			...
			if(viewCenter[dim_x] + (x - 40) < 0 ||
			   viewCenter[dim_x] + (x - 40) >= 160 ||
			   viewCenter[dim_y] + (y - 11) < 0 ||
			   viewCenter[dim_y] + (y - 11) >= 105)
			{
				addch(' ');
			} 
			else if(charxy(viewCenter[dim_x] + (x - 40), viewCenter[dim_y] + (y - 11)) )
			{	
				addch(charxy(viewCenter[dim_x] + (x - 40), viewCenter[dim_y] + (y - 11))->symbol );
			}
			else
			{
				switch(mapxy(viewCenter[dim_x] + (x - 40),viewCenter[dim_y] + (y - 11))) 
				{
					case ter_wall:
					case ter_wall_immutable:
						addch(' ');
...
void gen_stairs(dungeon_t *d, int numStairs)
{	
	/* Place 3 stairs */
	int currY;
	int currX;
	...
	{
		currRoom = rand_range(0, d->num_rooms);
		currX = rand_range(d->rooms[currRoom].position[dim_x] + 1, d->rooms[currRoom].position[dim_x] + d->rooms[currRoom].size[dim_x] - 1);
		currY = rand_range(d->rooms[currRoom].position[dim_y] + 1, d->rooms[currRoom].position[dim_y] + d->rooms[currRoom].size[dim_y] - 1);

		switch(rand_range(0,1))
		{
			case 0:
				mapxy(currX, currY) = left_stair;
				break;
			case 1:
				mapxy(currX, currY) = right_stair;
				break;
		}
