...
int main(int argc, char *argv[])
{
	...
	strcpy(path, "./test_dungeons/01.rlg327");
	...
	/* Represents boolean values for whether or not the user wants to
	save, load, both, or neither. */
	int save = 0;
	int load = 0;

	/* Checks if --load and/or --save is an argument */
	for(i = 0; i < argc; i++)
	{
	...
	//fread(pointer to read to, size of things to read, 
	//number of things to read, where to read from);

	if((fp = fopen(path, "r")))
	{
		printf("Opening file: %s\n", path);
		fread(&file_marker, sizeof(char), 12, fp); // File marker
		fread(&version, 4, 1, fp); // read file size
		version = be32toh(version);
		fread(&file_size, 4, 1, fp); // read file size
		file_size = be32toh(file_size);

		printf("File Marker: %s, Version: %u, Size: %u \n", file_marker, version, file_size);

		for(i = 0; i < DUNGEON_Y; i++)
		{
			for(j = 0; j < DUNGEON_X; j++)
			{
				fread(&d.hardness[i][j], 1, 1, fp); // read room positions
			}
		}

		/* While the x-position corner for a room is read, 
			read the other things and assign them.*/
		i = 0;
		...
		{			
			...
			d.rooms[i].position[0] = x_pos;
			d.rooms[i].position[1] = y_pos;
			d.rooms[i].size[0] = width;
			d.rooms[i].size[1] = height;
		} 

		for(i = 0; i < d.num_rooms; i++)
		{
			printf("x-pos: %d, y-pos: %d, width: %d, height: %d\n", d.rooms[i].position[0], d.rooms[i].position[1], d.rooms[i].size[0], d.rooms[i].size[1]);
		}

		uint8_t cell_hardness;

		for(i = 0; i < 105; i++)
		{	
			for(j = 0; j < 160; j++)
			{
				cell_hardness = d.hardness[i][j];
				if(cell_hardness)
				{
					dungeon[i][j] = ' ';
				}
		...
		{	
			curr_room = d.rooms[i];
			x_pos = curr_room.position[0];
			y_pos = curr_room.position[1];
			width = curr_room.size[0];
			height = curr_room.size[1];
			printf("%d, %d, %d, %d, %d\n", i, x_pos, y_pos, height, width);

			for(j = y_pos; j < height + y_pos; j++ )
			{
				for(k = x_pos; k < width + x_pos; k++)
				{
					dungeon[j][k] = '.'; 
				}
			}
