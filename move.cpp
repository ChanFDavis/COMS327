...
uint32_t move_pc(dungeon_t *d, uint32_t dir)
{
	...
	int16_t empty_slot = has_empty_slot(d->PC);


	if ((dir != '>') && (dir != '<') && (mappair(next) >= ter_floor)) {
		move_character(d, d->PC, next);
		io_update_offset(d);
		if(objpair(d->PC->position) && empty_slot >= 0)
		{
			io_queue_message("Walking forward, you see an item...");
			d->PC->inventory[empty_slot] = objpair(d->PC->position);
			io_queue_message("You've picked up the item and put it in inventory slot %d...", empty_slot);
			d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]] = 0;
			io_queue_message("Press any button to continue.");
		}
		dijkstra(d);
		dijkstra_tunnel(d);
